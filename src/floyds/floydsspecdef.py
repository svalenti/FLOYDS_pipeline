...
def correctfringing_auto(img0,imgN):
    ...
    from numpy import trapz, compress, argmin, argmax, column_stack, savetxt, array, float32
    from pyraf import iraf
    from pylab import plot,ion,clf
    ...
    iraf.specred.continuum('sky1.fits',output='tsky1.fits',line='*',type='difference', \
                               interact='yes',function='spline3',niterat=100,low_rej=4,high_re=5,  sample='7000:7550,7730:10000',order=10, ask='no')
    ...
    shift=floyds.floydsspecdef.checkwavelength_arc(xx1,yy1-min(yy1),xx2,yy2,'','',False)*(-1)  # shift fringing on std and fringing from flat
    print shift
    if abs(shift)>15: shift=0
    xx3=array(xx2,float)[:]
    yy3=yy2[:]
    xx4=xx3+shift
    ...
    floyds.util.delete('tsky11.fits')
    floyds.util.delete('sky2.fits')
    data1, hdr = pyfits.getdata(img0, 0, header=True)
    if 'GRISM' in hdr:    arm=hdr['GRISM']
    else:                 arm=''
    ...
    pyfits.writeto(img0, float32(data1), hdr)
...
def normflat(img):
    ...
    iraf.specred.continuum('sky1.fits',output='tsky1.fits',type='fit', \
                               interact='no',function='spline3',niterat=100,low_rej=0,high_re=0,  sample='*',order=_order1, ask='YES')
    iraf.specred.continuum('sky2.fits',output='tsky2.fits',type='fit', \
                               interact='no',function='chebyshev',niterat=100,low_rej=0,high_re=0,  sample='*',order=_order2, ask='YES')
    sss=iraf.specred.scombine('tsky1.fits,tsky2.fits',output='M'+img,combine='average',reject='none',scale='none',weight='none',Stdout=1)
...
def correctfringing(imgex,fringingmask):
    ...
    from pyraf import iraf
    import pyfits
    from numpy import float32, compress
    ...
    while _scale not in ['n','no']:
        ...
        try: 
            ...
            yynew=yy1/yy3
        lines.pop(0).remove()
    data1, hdr = pyfits.getdata(imgex, 0, header=True)
    data1[0]=array(yynew)
...
#    JDtoday=55928.00+date2num(datetime.datetime.utcnow())-matplotlib.dates.date2num(datetime.datetime(2012, 01, 01,12,0,0))
    ...
    if not os.path.isfile(_outputsens):
       ...
       if sample=='*':
           ...
           floyds.util.delete(_outputsens)
           iraf.specred.standard(input=standardfile,output=_outputstd, extinct=_extinctdir+_extinction,caldir=_caldir,\
                                     observa=_observatory,star_nam=refstar, airmass=_airmass, exptime=_exptime, interac=_interactive)
           iraf.specred.sensfunc(standard=_outputstd, sensitiv=_outputsens, extinct=_extinctdir+_extinction, ignorea='yes',\
                                     observa=_observatory,functio=_function, order=_order, interac=_interactive)
       else:
           _outputstd='std_'+str(readkey3(hdrs,'grism'))+'_'+str(readkey3(hdrs,'filter'))+'.fits'
           floyds.util.delete(_outputstd)
           iraf.specred.standard(input=standardfile,output=_outputstd, extinct=_extinctdir+_extinction,caldir=_caldir,\
           ...
           iraf.specred.sensfunc(standard=_outputstd, sensitiv='sens0.fits', extinct=_extinctdir+_extinction, ignorea='yes',\
                                     observa=_observatory,functio=_function, order=_order, interac=_interactive)
           sss='sens0.fits'
           ...
           for i in string.split(sss,','):   floyds.util.delete(i)
...
def telluric_atmo(imgstd):
   ...
   toforget = ['imfilter.gauss','specred.apall','longslit.identify','longslit.reidentify','specred.standard','onedspec.wspectext']
   for t in toforget: iraf.unlearn(t)
   import floyds
   from floyds.util import readhdr,readkey3, delete, readspectrum
   import os,string
...
#   iraf.noao.onedspec.bplot(imgstd, cursor=_cursor, spec2= imgstd, new_ima=imgout, overwri='yes')
   ...
   if _grism in ['red']:
       ...
       pyfits.writeto(_imgout, float32(data1), hdr)
       floyds.util.delete(atlas_smooto2)
       floyds.util.delete(atlas_smooth2o)
       floyds.util.delete(imgout)
   else:
...
def checkwavelength_arc(xx1,yy1,xx2,yy2, xmin, xmax,_interactive='yes'):
   ...
   if _interactive in ['YES','y','Y','yes','Yes',True]:
       ...
       legend(numpoints=1,markerscale=1.5)
       if xmin !='' and xmax!='':  xlim(xmin,xmax)
   return result
...
#    data[mask]=1
    pyfits.writeto(imgout, data, hdr)
    return imgout
...
def extractspectrum(img,dv,_ext_trace,_dispersionline,_interactive,_type,automaticex=False):
    ...
    if not os.path.isfile(imgex) and not os.path.isfile('database/ap'+re.sub('.fits','',img)) and not os.path.isfile('database/ap'+re.sub('.fits','',imgfast)):
            _new='yes'
            _extract='yes'
    else:
        if automaticex:
            if _interactive.upper() in ['YES','Y']:
                ...
                while answ not in ['o','n','s']:
                    ...
                    if not answ: answ='o'
                if answ=='o':   _new,_extract='no','yes'
                elif answ=='n':   _new,_extract='yes','yes'
            ...
            if _interactive.upper() in ['YES','Y']:
                ...
                while answ not in ['y','n']:
                    ...
                    if not answ: answ='y'
                if answ=='y':   _new,_extract='yes','yes'
                else:    _new,_extract='yes','no'
    ...
    if _extract=='yes':
        ...
        if _new=='no':
            if not os.path.isfile('database/ap'+re.sub('.fits','',img)):
                floyds.util.repstringinfile('database/ap'+re.sub('.fits','',imgfast),'database/ap'+re.sub('.fits','',img),re.sub('.fits','',imgfast),re.sub('.fits','',img))
            _find='no'
            _recenter='no'
            _edit='no'
            _trace='no'
...
#   extraction for agn or std, we always reset the parameters 
        ...
        floyds.util.repstringinfile('database/ap'+re.sub('.fits','',img),'database/ap'+re.sub('.fits','',imgfast),re.sub('.fits','',img),re.sub('.fits','',imgfast))

        data, hdr = pyfits.getdata(imgex, 0, header=True)
        xxex=arange(len(data[0][0]))
        aaex=readkey3(hdr,'CRVAL1')+(xxex)*readkey3(hdr,'CD1_1')
        floyds.util.updateheader(imgex,0,{'XMIN':[aaex[0],'min wavelength [Angstrom]'],'XMAX':[aaex[-1],'max wavelength [Angstrom]']})
...
def  choseflat(obj,listflat,setup,_JD0,_interactive):
    ...
    if _interactive:
        for flat in listflat:
            ...
            if not answ: answ='g'
            if answ in ['G','g','good','Good']:          flatgood.append(flat)
            elif answ in ['stop','S','Stop','s']:               break
    else:
...
def continumsub(imagefile,_order1,_order2):
    ...
    iraf.specred.continuum(imagefile,output='tsky.fits',type='difference', \
                       interact='no',function='legendre',niterat=300,low_rej=3,high_re=2,  sample='*',order=_order1, ask='YES')
    delete(imagefile)
    iraf.continuum('tsky.fits', output=imagefile, type='difference', \
                       interact='no',function='spline1',overrid='yes', niterat=10,low_rej=3,high_re=1, sample='*',order=_order2, ask='YES')
    delete('tsky.fits')
    return imagefile
##########################################
...
def floydsspecreduction(files,_interactive,_dobias,_doflat,_listflat,_listbias,_listarc,_cosmic,_ext_trace,_dispersionline,liststandard,listatmo,_automaticex,_classify=False,_verbose=False,smooth=1,fringing=1,_typefromuser='obj'):
    ...
    from numpy import array, arange, mean,pi,arccos,sin,cos,argmin,sqrt
    from pyfits import open as popen
    from pyraf import iraf
    import datetime
...
###############     flat correction  method 1 or 3  ###############
              ...
              if fringing in [1,3]:
                  ...
                  iraf.imcopy(img,imgn,verbose='no')
################    extraction         ####################################
              imgex=floyds.floydsspecdef.extractspectrum(imgn,dv,_ext_trace,_dispersionline,_interactive,tpe,automaticex=_automaticex)    
####################################################
...
############################################
              if not arcfile: 
                  ...
                  if answ in ['y','Y','YES','yes','Yes']:
                    ...
                    if identific:
                        ...
                        hedvec={'REFSPEC1':[re.sub('.fits','',arcfile),' reference arc'],'arc'+setup[0]:[re.sub('.fits','',arcfile),' reference arc']}
              ...
              if imgl:
                  ...
                  if setup not in wavecalib[tpe]: wavecalib[tpe][setup]=[imgl]
                  else: wavecalib[tpe][setup].append(imgl)
...
######################################################
                      ...
                      if setup not in sens: sens[setup]=[_outputsens2]
                      else: sens[setup].append(_outputsens2)              
     ...
     if tpe in wavecalib.keys():
      for setup in wavecalib[tpe].keys():
        for img in wavecalib[tpe][setup]:
            ...
            if not _atmo: 
                try:      _atmo=floyds.util.searchatmo(img,atmo[setup])[0]     # search in the reduced data
                except:   _atmo=''
            if not _atmo: 
                try:      _atmo=floyds.util.searchatmo(img,'')[0]     # search in the archive
                except:   _atmo=''
            ...
            if _sens:
                ...
                qqq=iraf.specred.calibrate(input=img , output= imgf, sensiti=_sens, extinct='yes',flux='yes',\
                                       extinction= _extinctdir+_extinction, observatory=_observatory, airmass=_airmass, ignorea='yes', exptime=_exptime, fnu='no')
                floyds.util.updateheader(imgf,0,{'sensfun'+setup[0][0]:[string.split(_sens,'/')[-1],'sensitivity curve']})
                hdr=floyds.readhdr(imgf)
                fileident=str(floyds.readkey3(hdr,'JD'))[:9]+' '+floyds.readkey3(hdr,'object')+' '+str(floyds.readkey3(hdr,'date-obs'))[:4]+'-'+\
                    str(floyds.readkey3(hdr,'date-obs'))[4:6]+'-'+str(floyds.readkey3(hdr,'date-obs'))[6:8]+' '+str(floyds.readkey3(hdr,'grism'))+' floyds.'+floyds.__version__
                ...
                if _atmo:
                   ...
                   iraf.specred.sarith(input1=imgf, op= '/', input2=_atmo, output=imge, w1='INDEF', w2='INDEF',format='multispec')
                   try:          iraf.imutil.imcopy(input=imgf+'[*,1,2]',output=imge+'[*,1,2]',verbose='no')
                   except: pass
                   try:          iraf.imutil.imcopy(input=imgf+'[*,1,3]',output=imge+'[*,1,3]',verbose='no')
                   except: pass	 
                   try:          iraf.imutil.imcopy(input=imgf+'[*,1,4]',output=imge+'[*,1,4]',verbose='no')
                   except: pass 
                   if imge not in outputfile: outputfile.append(imge)
                   floyds.util.updateheader(imge,0,{'ATMO'+setup[0][0]:[string.split(_atmo,'/')[-1],'']})                
                   imgin=imge                
    ...
    for obj in outputfile:
        if obj[-4:]=='fits':
          ...
          if '_e.fits' in obj:
              ...
              if MJD not in coppie: coppie[MJD]=[obj]
...
#    from numpy import interp as ninterp
    ...
    if 'NAXIS3' in hdr0 and 'NAXIS3' in hdr1:
        if hdr0['NAXIS3']==hdr1['NAXIS3']:    dimension=hdr1['NAXIS3']
        else:             dimension=1
    else:             dimension=1
    ...
    if scale:
        ...
        if integral0<integral1:   A0,A1=integral1/integral0,1
        else:                     A0,A1=1,integral0/integral1
    else: A0,A1=1,1
    a=abs(y11*A1-y01*A0)
    b=(y11*A1-y01*A0)
    a1=a[0:num]
...
def combineblusens(imglist,imgout='pippo.fits'):
    ...
    if len(where (yyr2-yyr1>0)):    xfix3=xxr[where (yyr2-yyr1>0)][0]
    else:                           xfix3=xxr[argmin(abs(yyr2-yyr1))]
    xxb=compress((xx2<3800),xx2)
    yyb2=compress((xx2<3800),yy2)
    yyb1=compress((xx2<3800),yy22)
    if len(where (yyb2-yyb1>0)):   xfix0=xxb[where (yyb2-yyb1>0)][-1]
    else:                          xfix0=xxb[argmin(abs(yyb2-yyb1))]
...
###################################    transform img, arc, flat
    ...
    if _arm=='red':
        iraf.specred.transform(input=img, output='t'+img, minput='', fitnames=re.sub('.fits','',imgrect), databas='database',\
                               x1='INDEF', x2='INDEF', y1='INDEF', y2='INDEF', dy=1, flux='yes', blank=0, logfile='logfile')
    else:
        iraf.specred.transform(input=img, output='t'+img, minput='', fitnames=re.sub('.fits','',imgrect), databas='database',\
                               x1='INDEF', x2='INDEF', y1='INDEF', y2='600', dy=1, flux='yes', blank=0, logfile='logfile')

    data, hdr = pyfits.getdata('t'+img, 0, header=True)
    ...
    if not hdr.get('HDRVER'):
        pyfits.writeto('t'+img, float32(data[0][ya:yb,xa:xb]),hdr)
    else:
        pyfits.writeto('t'+img, float32(data[ya:yb,xa:xb]),hdr)
    ...
    if _cosmic:
        floyds.cosmics.lacos('t'+img,output='',gain=_gain,readn=_rdnoise,xorder=9,yorder=9,sigclip=4.5,sigfrac=0.5,objlim=1,verbose=True,interactive=True)
...
#                                   x1='INDEF', x2='INDEF', y1='INDEF', y2='INDEF', flux='yes',  blank=0, logfile='logfile')
        ...
        if not hdr.get('HDRVER'):
            ...
            pyfits.writeto('t'+arcfile, float32(data[ya:yb,xa:xb]),hdr)
...
#                               x1='INDEF', x2='INDEF', y1='INDEF', y2='INDEF', flux='yes',  blank=0, logfile='logfile')#, mode='h')
        ...
        if not hdr.get('HDRVER'):
            ...
            pyfits.writeto('t'+flatfile, float32(data[ya:yb,xa:xb]),hdr)
...
def fringing_classicmethod(flatfile,img,_inter,_sample,_order,arm):
    ...
    if arm=='red':
        ...
        iraf.specred.response(flatfile,normaliz=flatfile+'[*,1:'+str(int(ydim)-1)+']',\
                              ...
                              function='spline3', low_rej=3,high_rej=3,order=_order, niterat=20, grow=0, graphic='stdgraph')
        ...
        iraf.specred.response(flatfile,normaliz=flatfile+'[*,1:'+str(int(ydim)-1)+']',\
                              ...
                              function='spline3', low_rej=3,high_rej=3,order=40, niterat=20, grow=0, graphic='stdgraph')
    floyds.util.delete('n'+flatfile)
    floyds.util.delete('n'+img)
    ...
    floyds.util.delete('n'+img)
    data, hdr = pyfits.getdata(img, 0, header=True)
    datan, hdrn = pyfits.getdata('n'+flatfile, 0, header=True)
#########
    mask=datan==0
    datan[mask]=1
...
#########
    _grism=hdr['grism']
    pyfits.writeto('n'+img,data2, hdr)
...
def  applyflat(img,flatimg,output='',scale='',shift=''):
    ...
    data, hdr = pyfits.getdata(img, 0, header=True)
    datan, hdrn = pyfits.getdata(flatimg, 0, header=True)
    if 'GRISM' in hdr:    arm=hdr['GRISM']
    else:                 arm=''
    if scale=='' and shift=='':
      y=data.mean(1)
      ...
      if np.argmax(y) < 80 and np.argmax(y) > 15:
          ...
          pyfits.writeto('_sky1.fits', np.float32(y2-yy2), hdr)
          pyfits.writeto('_sky2.fits', np.float32(yn), hdr)
          ...
          data1, hdr1 = pyfits.getdata('tsky1.fits', 0, header=True)       #  fringing on the standrad
          yy1=data1
          xx1=np.arange(len(yy1))
          data11, hdr11 = pyfits.getdata('sky1.fits', 0, header=True)     #  standard
          yy11=data11
          xx11=np.arange(len(yy11))
          data111, hdr111 = pyfits.getdata('tsky11.fits', 0, header=True)     #  continuum
          yy111=data111
          xx111=np.arange(len(yy111))
          data2, hdr2 = pyfits.getdata('sky2.fits', 0, header=True)         #  fringing form flat
          yy2=data2
          xx2=np.arange(len(yy2))
          shift=floyds.floydsspecdef.checkwavelength_arc(xx1,yy1,xx2,yy2,'','',False)*(-1.)    #   shift fringing
...
####################################
    ...
    pyfits.writeto(output, np.float32(datanew2), hdr)
...
def fringing_classicmethod2(flatfile,img,_inter,_sample,_order,arm):
    ...
    data, hdr = pyfits.getdata(img, 0, header=True)
    datan, hdrn = pyfits.getdata('n'+flatfile, 0, header=True)
    pyfits.writeto('n'+img, float32(data/datan), hdr)
