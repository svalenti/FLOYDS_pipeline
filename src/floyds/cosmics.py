...
#    iraf.imarith(skymod,"+",med5,med5)
    ...
    iraf.blkavg(lapla,deriv2,2,2,option="average")
      # create noise model
    iraf.imutil.imexpr(expr='sqrt(a*'+str(gain)+'+'+str(readn)+'**2)/'+str(gain),a=med5,output=noise,verbose='no')
...
def clean_image(img,cleanimg):
    ...
    try:
	    c = floyds.cosmics.cosmicsimage(array, gain=readkey3(header,'gain'), readnoise=readkey3(header,'ron'), sigclip = 5.0, sigfrac = 0.3, objlim = 5.0,verbose=False)
...
def my_convolve_with_FFT2(image1, kernel):
    ...
    r = 2**pr2
    c = 2**pc2
    fftimage = np.fft.fft2(image1, s= (r,c)) * np.fft.fft2(kernel[::-1,::-1], s=(r,c))
...
def lacos_im(_input,_output='clean.fits',outmask='mask.fits',gain=1.3,readn=9,xorder=9,yorder=9,sigclip=4.5,sigfrac=0.5,objlim=1,skyval=0,niter=2,verbose=True,interactive=False):
	...
	delete(oldoutput)
	if skyval>0: 	    arrayoldoutput=arrayinput+skyval
	else:   arrayoldoutput=arrayinput
	...
	while stop=='no':
		...
		# create noise model
		delete(noise)
		iraf.imutil.imexpr(expr='sqrt(a*'+str(usegain)+'+'+str(readn)+'**2)/'+str(usegain),a=med5,output=noise,verbose='no')
		# divide Laplacian by noise model
		delete(sigmap)
